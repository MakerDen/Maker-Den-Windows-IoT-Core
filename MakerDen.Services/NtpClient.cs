...
namespace MakerDen
{
    public class NtpClient
    {
        ...
        public async Task<DateTime> GetNetworkTimeAsync(TimeSpan timeout)
        {
            ...
            {
                ct.Token.Register(() => _resultCompletionSource.TrySetCanceled(), false);
        ...
        private void OnSocketMessageReceived(DatagramSocket sender, DatagramSocketMessageReceivedEventArgs args)
        {
            try
            {
                ...
                sender.MessageReceived -= OnSocketMessageReceived;
                using (var reader = args.GetDataReader())
                {
                    byte[] response = new byte[48];
                    reader.ReadBytes(response);
                    _resultCompletionSource.SetResult(ParseNetworkTime(response));
                }
            }
            catch (Exception ex)
            {
                _resultCompletionSource.SetException(ex);
            }
        }
