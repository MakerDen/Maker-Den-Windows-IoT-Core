...
namespace MakerDen.Json
{
    public class JSONWriter : ByteArrayBuilder
    {
        ...
        public void AddProperty(string name, double value, uint decimalPlaces)
        {
            AddPropertyName(name);
            AddPropertyValue(value, decimalPlaces);
        }
        ...
        private void AddPropertyValue(double[] value, uint decimalPlaces)
        {
            Append("[");
            for (int i = 0; i < value.Length; i++)
            {
                //Append(value[i].ToString(format));
                Append(Math.Round(value[i], (int)decimalPlaces).ToString());
                if (i < value.Length - 1)
                {
                    Append(",");
                }
        ...
        private void AddPropertyValue(double value, uint decimalPlaces)
        {
            Append(Math.Round(value, (int)decimalPlaces).ToString());
        }
